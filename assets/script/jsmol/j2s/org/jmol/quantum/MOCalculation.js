...
Clazz.load (["org.jmol.api.QuantumCalculationInterface", "org.jmol.quantum.QuantumCalculation"], "org.jmol.quantum.MOCalculation", ["org.jmol.constant.EnumQuantumShell", "org.jmol.util.Logger"], function () {
...
}, org.jmol.quantum, "MOCalculation", org.jmol.quantum.QuantumCalculation, org.jmol.api.QuantumCalculationInterface);
...
function () {
Clazz.superConstructor (this, org.jmol.quantum.MOCalculation, []);
});
...
org.jmol.util.Logger.info ("Normalizing AOs: " + doNormalize + " slaters:" + (slaters != null));
...
Clazz.superCall (this, org.jmol.quantum.MOCalculation, "initialize", [nX, nY, nZ, points]);
...
org.jmol.util.Logger.warn ("calculation type not identified -- continuing");
return true;
}this.nwChemMode = (this.calculationType.indexOf ("NWCHEM") >= 0);
if (this.nwChemMode) org.jmol.util.Logger.info ("Normalization of contractions (NWCHEM)");
if (this.calculationType.indexOf ("+") >= 0 || this.calculationType.indexOf ("*") >= 0) {
org.jmol.util.Logger.warn ("polarization/diffuse wavefunctions have not been tested fully: " + this.calculationType + " -- continuing");
}if (this.calculationType.indexOf ("?") >= 0) {
org.jmol.util.Logger.warn ("unknown calculation type may not render correctly -- continuing");
} else if (this.points == null) {
org.jmol.util.Logger.info ("calculation type: " + this.calculationType + " OK.");
}return true;
...
this.basisType = org.jmol.constant.EnumQuantumShell.getItem (shell[1]);
...
break;
default:
org.jmol.util.Logger.warn (" Unsupported basis type for atomno=" + (this.atomIndex + 1) + ": " + this.basisType.tag);
break;
}
}, $fz.isPrivate = true, $fz), "~N");
...
if (org.jmol.util.Logger.debugging) org.jmol.util.Logger.debug ("\t\t\tnormalization for l=" + el + " nGaussians=" + this.nGaussians + " is " + sum);
return sum;
...
org.jmol.util.Logger.debug ("\n\t\t\tprocessShell: " + shell + " type=" + org.jmol.constant.EnumQuantumShell.getQuantumShellTag (shell) + " nGaussians=" + this.nGaussians + " atom=" + this.atomIndex);
this.doShowShellType = false;
}if (org.jmol.util.Logger.isActiveLevel (6)) for (var ig = 0; ig < this.nGaussians; ig++) {
var alpha = this.gaussians[this.gaussianPtr + ig][0];
var c1 = this.gaussians[this.gaussianPtr + ig][1];
org.jmol.util.Logger.debug ("\t\t\tGaussian " + (ig + 1) + " alpha=" + alpha + " c=" + c1);
}
var so = org.jmol.quantum.MOCalculation.getShellOrder (shell);
for (var i = 0; i < so.length; i++) {
var c = this.coeffs[i];
org.jmol.util.Logger.debug ("MO coeff " + so[i] + " " + (this.map[i] + this.moCoeff - this.map.length + i + 1) + "\t" + c + "\t" + this.thisAtom.atom);
}
}, $fz.isPrivate = true, $fz), "~N");
c$.getShellOrder = Clazz.defineMethod (c$, "getShellOrder", 
($fz = function (i) {
return (i < 0 || i > org.jmol.quantum.MOCalculation.shellOrder.length ? null : org.jmol.quantum.MOCalculation.shellOrder[i]);
}, $fz.isPrivate = true, $fz), "~N");
...
org.jmol.util.Logger.info ("Integrated density = " + this.integration);
});
