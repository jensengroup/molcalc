...
}, org.jmol.minimize, "Minimizer", null, org.jmol.api.MinimizerInterface);
...
org.jmol.util.ArrayUtil.swapInt (atoms, 1, nAtoms);
if (nAtoms == 4) org.jmol.util.ArrayUtil.swapInt (atoms, 2, 3);
}var id = org.jmol.util.Escape.escape (atoms);
...
if (this.pFF == null) {
org.jmol.util.Logger.error (org.jmol.i18n.GT._ ("Could not get class for force field {0}", ff));
return false;
}org.jmol.util.Logger.info ("minimize: initializing " + this.pFF.name + " (steps = " + steps + " criterion = " + crit + ") ...");
if (bsSelected.cardinality () == 0) {
org.jmol.util.Logger.error (org.jmol.i18n.GT._ ("No atoms selected -- nothing to do!"));
return false;
}this.atoms = this.viewer.getModelSet ().atoms;
this.bsAtoms = org.jmol.util.BitSetUtil.copy (bsSelected);
if (bsFixed != null) this.bsAtoms.or (bsFixed);
this.atomCount = this.bsAtoms.cardinality ();
var sameAtoms = org.jmol.util.BitSetUtil.areEqual (bsSelected, this.bsSelected);
this.bsSelected = bsSelected;
if (!sameAtoms) this.pFF.clear ();
if ((!sameAtoms || !org.jmol.util.BitSetUtil.areEqual (bsFixed, this.bsFixed)) && !this.setupMinimization ()) {
this.clear ();
return false;
}if (steps > 0) {
this.bsTaint = org.jmol.util.BitSetUtil.copy (this.bsAtoms);
org.jmol.util.BitSetUtil.andNot (this.bsTaint, bsFixed);
...
this.minAtoms[pt] =  new org.jmol.minimize.MinAtom (pt, atom, [atom.x, atom.y, atom.z], this.atomCount);
this.minAtoms[pt].sType = atom.getAtomName ();
}
org.jmol.util.Logger.info (org.jmol.i18n.GT._ ("{0} atoms will be minimized.", "" + this.atomCount));
org.jmol.util.Logger.info ("minimize: getting bonds...");
this.bonds = this.viewer.getModelSet ().getBonds ();
this.rawBondCount = this.viewer.getModelSet ().getBondCount ();
this.getBonds ();
org.jmol.util.Logger.info ("minimize: getting angles...");
this.getAngles ();
org.jmol.util.Logger.info ("minimize: getting torsions...");
this.getTorsions ();
...
org.jmol.util.Logger.error (org.jmol.i18n.GT._ ("could not setup force field {0}", this.ff));
...
($fz = function () {
var bondInfo =  new java.util.ArrayList ();
this.bondCount = 0;
var i1;
var i2;
...
bondInfo.add ( new org.jmol.minimize.MinBond (i, this.bondCount++, this.atomMap[i1], this.atomMap[i2], bondOrder, 0, null));
}
...
function () {
var vAngles =  new java.util.ArrayList ();
var atomList;
var ic;
for (var i = 0; i < this.bondCount; i++) {
...
vAngles.add ( new org.jmol.minimize.MinAngle ([ia, ib, ic, i, this.minAtoms[ib].getBondIndex (j)]));
...
vAngles.add ( new org.jmol.minimize.MinAngle ([ic, ia, ib, this.minAtoms[ia].getBondIndex (j), i]));
this.minAtoms[ic].bsVdw.clear (ib);
}
}}
this.minAngles = vAngles.toArray ( new Array (vAngles.size ()));
org.jmol.util.Logger.info (this.minAngles.length + " angles");
});
Clazz.defineMethod (c$, "getTorsions", 
function () {
var vTorsions =  new java.util.ArrayList ();
var id;
...
id = atomList[j];
if (id != ia && id != ib) {
vTorsions.add ( new org.jmol.minimize.MinTorsion ([ia, ib, ic, id, angle[3], angle[4], this.minAtoms[ic].getBondIndex (j)]));
...
id = atomList[j];
if (id != ic && id != ib) {
vTorsions.add ( new org.jmol.minimize.MinTorsion ([ic, ib, ia, id, angle[4], angle[3], this.minAtoms[ia].getBondIndex (j)]));
this.minAtoms[Math.min (ic, id)].bs14.set (Math.max (ic, id));
}}
}}
this.minTorsions = vTorsions.toArray ( new Array (vTorsions.size ()));
org.jmol.util.Logger.info (this.minTorsions.length + " torsions");
});
Clazz.defineMethod (c$, "getForceField", 
function (ff) {
if (this.pFF == null || !ff.equals (this.ff)) {
if (ff.equals ("UFF")) {
this.pFF =  new org.jmol.minimize.forcefield.ForceFieldUFF (this);
} else if (ff.equals ("MMFF")) {
this.pFF =  new org.jmol.minimize.forcefield.ForceFieldMMFF (this);
} else {
this.pFF =  new org.jmol.minimize.forcefield.ForceFieldUFF (this);
ff = "UFF";
}this.ff = ff;
...
function () {
try {
org.jmol.util.Logger.info ("minimizer: startMinimization");
...
org.jmol.util.Logger.error ("minimization error viewer=" + this.viewer + " pFF = " + this.pFF);
return false;
} else {
throw e;
}
...
org.jmol.util.Logger.info ("minimizer: endMinimization");
});
...
function (msg, isEcho) {
if (this.isSilent) org.jmol.util.Logger.info (msg);
...
var ff =  new org.jmol.minimize.forcefield.ForceFieldMMFF (this);
