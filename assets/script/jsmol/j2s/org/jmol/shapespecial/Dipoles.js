...
Clazz.load (["org.jmol.shape.Shape", "org.jmol.util.Point3f"], "org.jmol.shapespecial.Dipoles", ["java.lang.Float", "java.util.ArrayList", "$.Hashtable", "org.jmol.script.Token", "org.jmol.shapespecial.Dipole", "org.jmol.util.ArrayUtil", "$.BitSetUtil", "$.Colix", "$.Logger", "$.StringXBuilder", "$.TextFormat", "$.Vector3f"], function () {
...
}, org.jmol.shapespecial, "Dipoles", org.jmol.shape.Shape);
Clazz.prepareFields (c$, function () {
this.dipoles =  new Array (4);
this.startCoord =  new org.jmol.util.Point3f ();
this.endCoord =  new org.jmol.util.Point3f ();
});
Clazz.defineMethod (c$, "setProperty", 
function (propertyName, value, bs) {
if ("init" === propertyName) {
this.tempDipole =  new org.jmol.shapespecial.Dipole ();
...
if (this.currentDipole != null) org.jmol.util.Logger.debug ("current dipole: " + this.currentDipole.thisID);
return;
}if ("calculate" === propertyName) {
this.calculatedDipole = this.viewer.calculateMolecularDipole ();
org.jmol.util.Logger.info ("calculated molecular dipole = " + this.calculatedDipole + " " + (this.calculatedDipole == null ? "" : "" + this.calculatedDipole.length ()));
return;
...
org.jmol.util.Logger.debug ("current dipole now " + this.currentDipole.thisID);
this.tempDipole = this.currentDipole;
if (thisID.equals ("molecular")) {
var v = this.calculatedDipole;
if (v == null) {
v = this.viewer.getModelDipole ();
org.jmol.util.Logger.info ("file molecular dipole = " + v + " " + (v != null ? "" + v.length () : ""));
...
v =  new org.jmol.util.Vector3f ();
}this.tempDipole.set (org.jmol.util.Point3f.new3 (0, 0, 0), org.jmol.util.Vector3f.new3 (-v.x, -v.y, -v.z));
...
this.tempDipole.set (this.startCoord, org.jmol.util.Point3f.new3 (0, 0, 0), this.dipoleValue);
...
var atomset = value;
if (this.atomIndex1 >= 0 && org.jmol.util.BitSetUtil.cardinalityOf (atomset) == 1) {
...
this.dipoles = org.jmol.util.ArrayUtil.deleteElements (this.dipoles, i, 1);
this.dipoleCount--;
}
this.currentDipole = null;
return;
...
for (var i = this.dipoleCount; --i >= 0; ) if (!bondOnly || this.isBondDipole (i)) if (this.wildID == null || org.jmol.util.TextFormat.isMatch (this.dipoles[i].thisID.toUpperCase (), this.wildID, true, true)) this.setProperty (tok, this.dipoles[i], iValue, fValue);
...
}, $fz.isPrivate = true, $fz), "org.jmol.modelset.Atom,org.jmol.modelset.Atom,~N,~N");
...
}, $fz.isPrivate = true, $fz), "org.jmol.shapespecial.Dipole,~S");
Clazz.defineMethod (c$, "getDipoleIndex", 
...
}, $fz.isPrivate = true, $fz), "org.jmol.modelset.Atom,org.jmol.modelset.Atom,~B");
...
($fz = function (thisID, dipoleInfo) {
this.dipoles = org.jmol.util.ArrayUtil.ensureLength (this.dipoles, this.dipoleCount + 1);
if (thisID == null || thisID.length == 0) thisID = "dipole" + (this.dipoleCount + 1);
var d = this.dipoles[this.dipoleCount++] =  new org.jmol.shapespecial.Dipole (this.viewer.getCurrentModelIndex (), thisID, dipoleInfo, this.colix, 5, true);
return d;
}, $fz.isPrivate = true, $fz), "~S,~S");
...
if (this.currentDipole != null) org.jmol.util.Logger.info (" current = " + this.currentDipole + this.currentDipole.origin);
if (this.tempDipole != null) org.jmol.util.Logger.info (" temp = " + this.tempDipole + " " + this.tempDipole.origin);
}, $fz.isPrivate = true, $fz), "~S");
...
var dipole = this.dipoles[i];
if (dipole.isValid) {
if (modelCount > 1 && dipole.modelIndex != thisModel) org.jmol.shape.Shape.appendCmd (s, "frame " + this.viewer.getModelNumberDotted (thisModel = dipole.modelIndex));
s.append (dipole.getShapeState ());
org.jmol.shape.Shape.appendCmd (s, this.getColorCommandUnk ("dipole", dipole.colix));
}}
return s.toString ();
});
