...
this.nXX = 0;
this.symLine = null;
if (!Clazz.isClassDefined ("org.jmol.adapter.readers.quantum.AdfReader.SymmetryData")) {
org.jmol.adapter.readers.quantum.AdfReader.$AdfReader$SymmetryData$ ();
}
Clazz.instantialize (this, arguments);
}, org.jmol.adapter.readers.quantum, "AdfReader", org.jmol.adapter.readers.quantum.SlaterReader);
...
var tokens = org.jmol.adapter.smarter.AtomSetCollectionReader.getTokensStr (this.line.substring (this.line.indexOf ("Energy:")));
...
this.readMolecularOrbitals (org.jmol.adapter.smarter.AtomSetCollectionReader.getTokensStr (this.symLine)[1]);
return true;
}return true;
});
...
($fz = function () {
this.vSymmetries =  new java.util.ArrayList ();
...
var tokens = org.jmol.adapter.smarter.AtomSetCollectionReader.getTokensStr (syms);
for (var i = 0; i < tokens.length; i++) {
var sd = Clazz.innerTypeInstance (org.jmol.adapter.readers.quantum.AdfReader.SymmetryData, this, null, index++, tokens[i]);
this.htSymmetries.put (tokens[i], sd);
this.vSymmetries.add (sd);
}
}, $fz.isPrivate = true, $fz));
...
org.jmol.util.Logger.info (sd.sym);
...
tokens = org.jmol.adapter.smarter.AtomSetCollectionReader.getTokensStr (data);
...
this.slaterArray[ptBF] =  new org.jmol.quantum.SlaterData (atomList[i], x, y, z, r, zeta, 1);
...
this.readLine ();
sd.mos = org.jmol.util.ArrayUtil.createArrayOfHashtable (sd.nSFO);
...
this.setMO (mo);
}, $fz.isPrivate = true, $fz), "org.jmol.adapter.readers.quantum.AdfReader.SymmetryData,~N,~N,~N");
...
Clazz.instantialize (this, arguments);
}, org.jmol.adapter.readers.quantum.AdfReader, "SymmetryData");
Clazz.makeConstructor (c$, 
function (a, b) {
org.jmol.util.Logger.info ("ADF reader creating SymmetryData " + b + " " + a);
this.index = a;
this.sym = b;
}, "~N,~S");
c$ = Clazz.p0p ();
