...
}, org.jmol.adapter.readers.quantum, "PsiReader", org.jmol.adapter.readers.quantum.MOReader);
Clazz.prepareFields (c$, function () {
this.atomNames =  new java.util.ArrayList ();
this.shellsByUniqueAtom =  new java.util.ArrayList ();
...
if (isInitial) {
this.atomNames.add (tokens[0]);
if (tokens[0].length <= 2) atom.elementNumber = org.jmol.api.JmolAdapter.getElementNumber (tokens[0]);
} else {
...
slatersByUniqueAtom =  new java.util.ArrayList ();
var nGaussians = 0;
...
nGaussians++;
gdata.add ([tokens[ipt], tokens[ipt + 1]]);
slater[2] = nGaussians;
}
if (slater != null) {
slatersByUniqueAtom.add (slater);
}this.shellsByUniqueAtom.add (slatersByUniqueAtom);
...
tokens = org.jmol.adapter.smarter.AtomSetCollectionReader.getStrings (this.readLine ().substring (21), nThisLine, 10);
for (var i = 0; i < nThisLine; i++) {
...
org.jmol.util.Logger.error ("Error reading Psi3 file molecular orbitals at line: " + this.line);
break;
} else {
throw e;
}
